...
func (c *ConfigSourceTest) TestUtils_Properties_GetBool(t *testing.T) {
	...
	Convey("测试get bool", func() {
		...
		Convey("k12(yEs) is true", func() {
			v, _ := p.GetBool("k12")
			So(v, ShouldNotBeNil)
			So(v, ShouldBeTrue)
		})
		Convey("k13(y) is true", func() {
			v, _ := p.GetBool("k13")
			So(v, ShouldNotBeNil)
			So(v, ShouldBeTrue)
		})
		Convey("k14(yes) is true", func() {
			v, _ := p.GetBool("k14")
			So(v, ShouldNotBeNil)
			So(v, ShouldBeTrue)
		})
		Convey("k15(YES) is true", func() {
			v, _ := p.GetBool("k15")
			So(v, ShouldNotBeNil)
			So(v, ShouldBeTrue)
		})
		Convey("k16(n) is false", func() {
			v, _ := p.GetBool("k16")
			So(v, ShouldNotBeNil)
			So(v, ShouldBeFalse)
		})
		Convey("k17(n) is false", func() {
			v, _ := p.GetBool("k17")
			So(v, ShouldNotBeNil)
			So(v, ShouldBeFalse)
		})
		Convey("k18(NO) is false", func() {
			v, _ := p.GetBool("k18")
			So(v, ShouldNotBeNil)
			So(v, ShouldBeFalse)
		})
		Convey("k19(no.txt) is false", func() {
			v, _ := p.GetBool("k19")
			So(v, ShouldNotBeNil)
			So(v, ShouldBeFalse)
		})
