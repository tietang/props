...
func (ccs *CompositeConfigSource) Backup() {
	...
	ccs.backFileName = filepath.Join(dir, BackupFile)
	if PathExists(ccs.backFileName) {
		newFileName := filepath.Join(dir, BackupFile+"."+time)
		err := os.Rename(ccs.backFileName, newFileName)
		if err != nil {
			log.Error(err)
			log.Error("重命名本地文件失败:"+ccs.backFileName, " to ", newFileName)
			return
		}
	}
	os.MkdirAll(filepath.Dir(ccs.backFileName), os.ModePerm)
	f, err := os.Create(ccs.backFileName)
	if err != nil {
		log.Error(err)
		log.Error("备份到本地文件失败:" + ccs.backFileName)
		return
	}
	defer f.Close()
	for _, k := range ccs.Keys() {
		v := ccs.GetDefault(k, "")
		f.WriteString(k)
		f.WriteString("=")
		f.WriteString(v)
