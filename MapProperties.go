...
func (p *MapProperties) GetDefault(key, defVal string) (string, error) {
    if v, ok := p.values[key]; ok {
        return v, nil
    }
    return defVal, errors.New("return default value, not exists for key: " + key)
}
...
func (p *MapProperties) GetIntDefault(key string, defVal int) (int, error) {
    if v, ok := p.values[key]; ok {
        if v, err := strconv.Atoi(v); err == nil {
            return v, nil
        }
...
func (p *MapProperties) GetBoolDefault(key string, defVal bool) (bool, error) {
    if v, ok := p.values[key]; ok {
        if v, err := strconv.ParseBool(v); err == nil {
            return v, nil
        }
...
func (p *MapProperties) GetFloat64Default(key string, defVal float64) (float64, error) {
    if v, ok := p.values[key]; ok {
        if v, err := strconv.ParseFloat(v, 64); err == nil {
            return v, nil
        }
